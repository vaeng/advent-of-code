...
AOC_DIR = Path(__file__).absolute().parent
...
README_PATH = AOC_DIR.parent / "README.md"
...
DayScores = namedtuple("DayScores", [
                       "time1", "rank1", "score1", "time2", "rank2", "score2"], defaults=[None] * 3)
...
def parse_leaderboard(leaderboard_path: Path) -> dict[int, DayScores]:
    ...
    with open(leaderboard_path) as file:
        ...
        assert len(
            matches) == 1, f"Found {'no' if len(matches) == 0 else 'more than one'} leaderboard?!"
...
def request_leaderboard(year: int) -> dict[int, DayScores]:
    ...
    with open(SESSION_COOKIE_PATH) as cookie_file:
        ...
        data = requests.get(PERSONAL_LEADERBOARD_URL.format(
            year=year), cookies={"session": session_cookie}).text
...
def draw_star(drawer: ImageDraw, at: tuple[int, int], size=9, color="#ffff0022", num_points=5):
    ...
    for angle in [diff * i - math.pi / 2 for i in range(num_points * 2)]:
        ...
        points.append((at[0] + math.cos(angle) * factor,
                      at[1] + math.sin(angle) * factor))
...
def generate_day_tile_image(day: str, year: str, languages: list[str], day_scores: DayScores | None) -> Path:
    ...
    image = get_alternating_background(languages, not (
        day_scores is None or day_scores.time2 is None))
    ...
    drawer.text((0, 74), lang_as_str, fill=font_color,
                align="left", font=secondary_font(lang_font_size))
    ...
    for part in (1, 2):
        ...
        if day_scores is not None and time is not None:
            drawer.text(
                (104, -5 + y), f"P{part} ", fill=font_color, align="left", font=main_font(25))
            if SHOW_CHECKMARK_INSTEAD_OF_TIME_RANK:
                ...
                drawer.line((160, 35 + y, 180, 15 + y),
                            fill=font_color, width=2)
                continue
            drawer.text((105, 25 + y), "time", fill=font_color,
                        align="right", font=secondary_font(10))
            drawer.text((105, 35 + y), "rank", fill=font_color,
                        align="right", font=secondary_font(10))
            drawer.text((143, 3 + y), format_time(time),
                        fill=font_color, align="right", font=secondary_font(18))
            drawer.text(
                (133, 23 + y), f"{rank:>6}", fill=font_color, align="right", font=secondary_font(18))
        else:
...
def handle_year(year: int, day_to_solutions: dict[int, list[str]]):
    ...
    with html.tag("h1", align="center"):
        stars = sum((ds.time1 is not None) + (ds.time2 is not None)
                    for ds in leaderboard.values() if ds is not None)
