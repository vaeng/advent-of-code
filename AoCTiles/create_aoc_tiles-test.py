...
YEAR_CONFIGS = {
    2022: {
        "year_folder_to_days": "Elixir/lib/advent_of_code",
        "day_pattern": r"day_\d{2}\.ex",
        "extensions": [".ex"],
        "branch": "main",
        "flat": True,
    },
...
OLD_YEAR_CONFIGS = {
    ...
    2020: {
        "year_folder_to_days": 'Clojure/src/advent_of_code/',
        "day_pattern": r"day([1-9][0-9]{0,1})\.rs",
        "extensions": [".rs"],
        "branch": "main",
        "flat": True,
    },
    2021: {
        "year_folder_to_days": 'Go/',
        "day_pattern": r"day([0-9][0-9])/day([0-9][0-9])\.go",
        "extensions": [".go"],
        "branch": "main",
        "flat": False,
    },
...
def get_solution_paths_dict_for_years() -> dict[int, dict[int, list[str]]]:
    ...
    for year_dir in sorted(
        ...
        for days in get_paths_matching_regex(
            year_dir / config["year_folder_to_days"], config["day_pattern"]
        ):
            day = find_first_number(days.name)
            solutions = (
                [days]
                if config["flat"]
                else sorted(find_recursive_solution_files(days))
            )

            solutions = [
                s for s in solutions if s.suffix in config["extensions"]]

            solutions = [solution.relative_to(AOC_DIR)
...
DayScores = namedtuple(
    "DayScores",
    ["time1", "rank1", "score1", "time2", "rank2", "score2"],
    defaults=[None] * 3,
)
...
def get_paths_matching_regex(path: Path, pattern: str):
    def sort_key(file):
        return int(re.sub(r"\D", "", file.name.split("/")[-1].split(".")[0]))

    return sorted(
        [p for p in path.iterdir() if re.fullmatch(pattern, p.name)], key=sort_key
    )
...
def parse_leaderboard(leaderboard_path: Path) -> dict[int, DayScores]:
    ...
    with open(leaderboard_path) as file:
        ...
        if no_stars in html:
            return {}
        matches = re.findall(rf"{start}(.*?){end}",
                             html, re.DOTALL | re.MULTILINE)
        assert (
            len(matches) == 1
        ), f"Found {'no' if len(matches) == 0 else 'more than one'} leaderboard?!"
...
def request_leaderboard(year: int) -> dict[int, DayScores]:
    ...
    if leaderboard_path.exists():
        ...
        has_no_none_values = all(itertools.chain(
...
def get_alternating_background(
    ...
    colors = [ImageColor.getrgb(extension_to_color[language])
        ...
        for x in range(image.width):
            for y in range(image.height):
                if fill_only_half and x / image.width + y / image.height > 1:
                    continue
                image.load()[x, y] = colors[(
                    (x + y) // stripe_width) % len(colors)]

    fill_with_colors(
        [NOT_COMPLETED_COLOR, darker_color(NOT_COMPLETED_COLOR)], False)
    if colors:
...
def draw_star(
    drawer: ImageDraw, at: tuple[int, int], size=9, color="#ffff0022", num_points=5
):
    ...
    for angle in [diff * i - math.pi / 2 for i in range(num_points * 2)]:
        ...
        points.append(
            (at[0] + math.cos(angle) * factor, at[1] + math.sin(angle) * factor)
        )
...
def generate_day_tile_image(
    ...
    drawer.text((3, -5), "Day", fill=font_color,
                align="left", font=main_font(20))
    drawer.text((1, -10), str(day), fill=font_color,
    ...
    for part in (1, 2):
        ...
        if day_scores is not None and time is not None:
            ...
            if SHOW_CHECKMARK_INSTEAD_OF_TIME_RANK:
                drawer.line((160, 35 + y, 150, 25 + y),
                            fill=font_color, width=2)
                drawer.line((160, 35 + y, 180, 15 + y),
                            fill=font_color, width=2)
                continue
            drawer.text(
                (105, 25 + y),
...
def handle_day(
    day: int, year: int, solutions: list[str], html: HTML, day_scores: DayScores | None
):
    languages = []
    for solution in solutions:
    ...
    if solutions:
        solution = "/".join(solutions[0].split("/")[1:])
        solution_link = f"https://github.com/jenarvaezg/aoc{year}/tree/{config['branch']}/{solution}"
    else:
        solution_link = ""
    ...
    with html.tag("a", href=str(solution_link)):
        html.tag("img", closing=False, src=str(
            day_graphic_path), width=TILE_WIDTH_PX)
